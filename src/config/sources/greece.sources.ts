...
/**
 * Source records for the Greece **destination** layer.
 *
 ...
 *
 * JURISDICTION. `jurisdiction` means *the applicant jurisdiction the source
 * governs*, not the authority's own country. The two Türkiye-scoped records
 * that used to live here are published by the Hellenic Republic but govern
 * applications lodged in Türkiye, so they belong to the filing-jurisdiction
 * layer and now sit in `tr-filing.sources.ts` (ADR-048).
 */
