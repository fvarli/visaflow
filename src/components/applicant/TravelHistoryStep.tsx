...
export function TravelHistoryStep() {
  ...
  const hints = state.applicant
    ? guidanceForStep(deriveApplicantGuidance(state.applicant), 'travelHistory')
    : []

  return (
    <div className="space-y-6">
      <Field label={t('disclosure.travelHistoryQuestion')}>
        <SegmentedControl<'no' | 'yes'>
          ariaLabel={t('disclosure.travelHistoryQuestion')}
          value={declared}
          onValueChange={setDeclared}
          options={[
            { value: 'no', label: t('disclosure.no') },
            { value: 'yes', label: t('disclosure.yes') },
          ]}
        />
      </Field>
      ...
      {showEditor ? (
        ...
        <GuidanceNote tone="neutral">
          {t('travelHistory.empty.description')}
        </GuidanceNote>
      )}
    </div>
  )
